...
export async function synthesize({ businessName, website, pageText, pageOk, search, todayShort, catalog }) {
...
Choose the RIGHT format ("kind") from the recommended product and the prospect's vertical:
- "wireframe": the recommended product is a website / online-presence / lead-capture build. Lay out the proposed pages/sections and what each one contains and why.
- "quote": the product is a concrete service or product with a known price (cleaning, install, contractor, signage, cards, hardware). Itemize the scope and the price.
- "proposal": everything else — a tight one-page proposal (problem -> solution -> what's included -> investment -> next step).

ABSOLUTE RULES:
1. PRICING. Use ONLY prices that appear VERBATIM in the CATALOG below. NEVER invent, estimate, round, or modify a number. If the matched product's price is "Custom pricing", blank, or a dash, write "Custom — contact for quote" and do NOT fabricate a figure.
2. EVIDENCE. Ground every line in the prospect's brief (their findings, vertical, opportunity). No generic filler a rep couldn't defend.
3. PRODUCTS. Only recommend product(s) that appear in the catalog.
4. OUTPUT. Return ONLY one JSON object in the exact schema. No prose, no markdown.`;
  ...
  return `Return JSON with this exact shape:
{
  ...
  "pricing": { "items": [ { "label": string, "detail": string, "amount": string } ], "total": string, "note": string } | null,
      // REQUIRED for "quote" and "proposal"; amounts VERBATIM from catalog. Use null only for a pure
      // wireframe where no price is being quoted yet. "total" may be "" if a single line already shows it.
...
export async function generateCloser(intel, catalog) {
  ...
  d.kind = ['wireframe', 'quote', 'proposal'].includes(d.kind) ? d.kind : 'proposal';
